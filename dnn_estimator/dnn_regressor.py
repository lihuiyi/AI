...
from dnn_estimator.csv_preprocessing import load_pkl, get_feature_column
...
label_standardScaler_obj_ = "label_standardScaler_obj"
...
def fit(train_path, eval_path, test_path, processing_pkl_Path, model_dir, hidden_units, dropout_rate,
    ...
    # 实例化模型
    regressor = model(hidden_units, model_dir, dropout_rate, processing_pkl_obj)
    # 训练模型、评估模型
    for i in range(train_epochs // epochs_per_eval):
        # 训练模型
        regressor.train(
            input_fn=lambda: train_input_fn(
                csv_path=train_path, pkl_obj=processing_pkl_obj, batch_size=batch_size,
        ...
        # 使用训练集评估模型
        train_result = regressor.evaluate(
            input_fn=lambda: train_input_fn(
    ...
    # 使用测试集评估模型
    test_result = regressor.evaluate(
        input_fn=lambda: eval_input_fn(
...
def predict(features, model_dir):
    ...
    # 加载预数据处理对象
    pkl_obj = load_pkl(processing_pkl_Path)
    ...
    # 实例化模型
    regressor = model(hidden_units, model_dir, dropout_rate, pkl_obj)
    # 预测
    predict_result = regressor.predict(
        input_fn=lambda: predict_input_fn(features, labels=None, pkl_obj=pkl_obj, batch_size=128)
    )
    # 解析预测结果
    pred_list = []
    for pred_dict in zip(predict_result):
        predictions = pred_dict[0]["predictions"]
        predictions = label_inverse_transform(pkl_obj, predictions)
...
def label_inverse_transform(pkl_obj, predictions):
    label_standardScaler_obj = pkl_obj[label_standardScaler_obj_]
    predictions = label_standardScaler_obj.inverse_transform(predictions)
    return predictions
