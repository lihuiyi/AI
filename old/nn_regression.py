...
class NN(NN_Config):
    ...
    def creat_ini_config(self , layers , config_name):
        ...
        options = [
            ...
            dict(enable_L2_regularizer=str(self.enable_L2_regularizer) ,
                 L2_regularizer_rate=str(self.L2_regularizer_rate)) ,
    ...
    def read_ini_config(self , model_path , config_name="神经网络参数配置.ini"):
    ...
    def get_weight(self , layers , stddev=1 , seed=1):
        '''
        功能：获取权重
        参数：
    ...
    # 学习率衰减
    def learning_rate_decay(self, X_train, global_step):
        '''
        功能：获取使用学习率衰减后的学习率
        参数：
        ...
        返回值：使用学习率衰减后的学习率
        '''
        if self.enable_learning_rate_decay is False:
            return self.learning_rate
        else:
            if self.decay_steps is None:
                self.decay_steps = X_train.shape[0] / self.batch_size  # 表示：衰减的步伐。完整的使用一遍训练数据所需要的迭代次数
                if self.decay_steps < 1000:
                    self.decay_steps = 1000
            # 每经过 decay_steps 次迭代，学习率衰减一次。参数
            learning_rate = tf.train.exponential_decay(
                self.learning_rate, global_step, self.decay_steps, self.decay_rate, staircase=self.staircase
            )
    ...
    def backward_propagation(self , y_true , output , L2_valuse , global_step):
        ...
        '''
        #定义损失函数
        mse = tf.reduce_mean(tf.square(y_true - output))  #均方误差
        # 是否启用 L2 正则化
        if L2_valuse is None:  # 不启用
            loss = mse
        else:  # 启用
            loss = mse + L2_valuse  # 最终损失函数
        # 优化损失函数
    ...
    def train(self , train_op , x , y_true , keep_prob , X_train , Y_train , X_validation , Y_validation , mse , global_step):
        """
        功能：训练神经网络，得到训练好的模型
            clf: 待训练的分类器
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.iteration):
                ...
                if ((i+1) % print_cycle == 0):
                    train_mse = sess.run(mse , feed_dict={x: batch_x , y_true: batch_y , keep_prob:self.keep_prob})
                    validation_mse = sess.run(mse , feed_dict={x: X_validation , y_true: Y_validation , keep_prob:self.keep_prob})
                    print(str(i+1) +  "   训练集："  + str(train_mse) +  "   验证集："  + str(validation_mse) ,end="")
    ...
    def fit(self , X_train, Y_train, X_validation, Y_validation):
        ...
        learning_rate = self.learning_rate_decay(X_train, global_step)  #获取学习率
        ...
        self.creat_ini_config(layers, config_name="神经网络参数配置.ini")
        self.train(train_op , x, y_true , keep_prob , X_train, Y_train, X_validation, Y_validation, mse , global_step) #训练神经网络
    ...
    def predict(self , model_path , X_test , Y_test , enable_moving_avg=False):
        ...
        with tf.Graph().as_default() as g:
            ...
            if enable_moving_avg is False:  # 不使用滑动平均
                ...
                moving_avg_rename_dict = moving_avg_obj.variables_to_restore()  #滑动平均变量重命名字典
            ...
            for i in range(len(models_path_list)):  # 对每一个模型进行预测
                ...
                with tf.Session() as sess:
                    ...
                    y_pred = sess.run(output , feed_dict={x: X_test , keep_prob:1}) #返回值是数字矩阵，是 numpy 类型
                    y_pred_list.append(y_pred)
                    if Y_test is not None:
                        test_mse = sess.run(mse , feed_dict={x: X_test , y_true: Y_test , keep_prob:1}) #返回值是数字矩阵，是 numpy 类型
                        print(model_path.split("-")[-1] + "次迭代，测试集均方误差：" , test_mse)
        if Y_test is not None:
            y_pred = y_pred_list
        else:
