...
class RNN(RNN_Config):
    def __init__(self):
        ...
        self.output_layer_unit = self.time_sequence  # 输出层单元数
    ...
    def forward_propagation(self , x , W , B , keep_prob):
        each_time_step_input = W[0].get_shape()[0]
        # 输入层与隐藏层之间是全连接关系，进行 Wx_plus_b 操作
        x = tf.reshape(x , [-1 , each_time_step_input])  # shape = [batch_size * time_step , each_time_step_input]
    ...
    def train(self , train_op , x , y_true , X_train , Y_train , X_validation , Y_validation , keep_prob , mse , global_step):
    ...
    def fit(self , X_train, Y_train, X_validation, Y_validation):
        ...
        y_true = tf.placeholder(tf.float32, [None , Y_train.shape[1]])
        ...
        self.train(train_op , x , y_true , X_train , Y_train , X_validation , Y_validation ,keep_prob , mse , global_step) #训练
        ...
        with tf.Graph().as_default() as g:
            ...
            for i in range(len(models_path_list)):  # 对每一个模型进行预测
                ...
                with tf.Session() as sess:
                    ...
                    y_pred = self.label_inverse_transform(y_pred) #标签反转：labelEncoder.inverse_transform()
                    y_pred_list.append(y_pred)
                    if Y_test is not None:
                        test_mse = sess.run(mse , feed_dict={x: X_test, y_true: Y_test, keep_prob: 1})
                        print(model_path.split("-")[-1] + "次迭代   测试集：" + str(test_mse))
            if Y_test is not None:
                y_pred = y_pred_list
            else:
    ...
    def plot_results(self , y_true , y_pred , image_path , linewidth=2.5):
        plt.figure()
        num = y_true.shape[0]
